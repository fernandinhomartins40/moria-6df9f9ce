...
class LegacyDataMigration {
  ...
  async migrateProducts() {
    ...
    try {
      // Buscar produtos que precisam migração
      const productsToMigrate = await this.prisma.product.findMany({
        where: {
          OR: [
            { isPublic: null },
            { status: null }
  ...
  async migrateServices() {
    ...
    try {
      const servicesToMigrate = await this.prisma.service.findMany({
        where: {
          OR: [
            { isPublic: null },
            { status: null }
  ...
  async migratePromotions() {
    ...
    try {
      const promotionsToMigrate = await this.prisma.promotion.findMany({
        where: {
          OR: [
            { isPublic: null },
            { status: null }
  ...
  async verifyIntegrity() {
    ...
    try {
      ...
      // Verificar dados órfãos ou inconsistentes
      const orphanedData = await this.prisma.product.count({
        where: {
          OR: [
            { isPublic: null },
            { status: null }
      ...
      if (orphanedData > 0) {
        console.warn(`  ⚠️ ${orphanedData} registros ainda precisam de migração`);
      } else {
