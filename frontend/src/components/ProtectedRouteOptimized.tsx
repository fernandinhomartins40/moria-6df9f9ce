// ========================================
// COMPONENTE DE ROTA PROTEGIDA OTIMIZADA - MORIA FRONTEND
// Componente otimizado para proteção de rotas com hooks customizados
// ========================================
...
export function ProtectedRouteOptimized({ children, requireAdmin = false }: ProtectedRouteProps) {
  const { customer, isAuthenticated, isLoading: isAuthLoading } = useAuth();
  const { isLoading: isAdminLoading, canAccessAdminFeatures } = useAdminAuth();
  const location = useLocation();

  const isLoading = isAuthLoading || (requireAdmin && isAdminLoading);
  ...
  // Requer admin e não tem permissão
