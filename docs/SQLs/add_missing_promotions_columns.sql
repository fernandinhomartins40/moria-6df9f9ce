...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        RAISE NOTICE 'Coluna type adicionada à tabela promotions';
    END IF;
    
    -- Adicionar coluna 'conditions' se não existir
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        RAISE NOTICE 'Coluna conditions adicionada à tabela promotions';
    END IF;
    
    -- Adicionar coluna 'max_discount' se não existir
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'promotions' AND column_name = 'max_discount') THEN
        ALTER TABLE promotions ADD COLUMN max_discount DECIMAL(10,2);
        RAISE NOTICE 'Coluna max_discount adicionada à tabela promotions';
    END IF;
    
    -- Verificar se start_date e end_date são NOT NULL (podem causar problemas)
    -- Alterar para permitir NULL se necessário
...
BEGIN
    ...
    RAISE NOTICE '- title (TEXT) - usado como name pela aplicação';
